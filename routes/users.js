  ...
  if(req.session.isLoggedIn){
    ...
    productHelper.getAllCategory().then((category)=>{
      productHelper.getAllProduct().then((product)=>{
        if(cartCount){
        res.render("User/home",{user:true,product,users,category,cartCount});
        }
      }).catch(()=>{
...
router.get('/productDetails/:id',async(req,res)=>{
  ...
  if(users){
    cartCount=await userHelpers.getCartCount(users._id)
  productHelper.getSingleProduct(proId).then((product)=>{
    res.render("User/product-details",{user:true,product,users,cartCount});
  }).catch(()=>{
    console.log('No Products found');
  })
}else{
...
router.get('/cart',isSignedIn,categories,async(req,res)=>{
  ...
  await user.getCart(req.session.user._id).then((data)=>{
    ...
    if(cartPro[0]){
       res.render('User/cart',{user:true,users,category,cartPro,cartCount,cart:true})
    }else{
...
router.get('/deleteCartProduct/:proId',(req,res)=>{
  let users=req.session.user._id
  user.deleteCartProduct(users,req.params.proId).then(()=>{
    res.json(response)
